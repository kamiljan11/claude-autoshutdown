...
STRINGS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "header.stalled": "! Sessions with nothing written for a long time: {n} (longest: {d}) - check whether one needs you",
        "btn.arm": "ARM",
        "btn.disarm": "DISARM",
        "btn.lang": "PL",
        # --- navigation -------------------------------------------------------------
        ...
        "log.stalled": "STUCK: session {name} (PID {pid}) has had an open turn and wrote nothing for {d}. Most often that means a permission prompt is waiting for you, but a single long-running tool call or a wait on an API rate limit looks the same from here. Nothing will shut down while the turn is open.",
    ...
    "pl": {
        ...
        "header.stalled": "! Sesje bez zapisu od dluzszego czasu: {n} (najdluzej: {d}) - sprawdz, czy ktoras nie czeka na Ciebie",
        "btn.arm": "UZBROJ",
        "btn.disarm": "ROZBROJ",
        "btn.lang": "EN",
        "nav.monitor": "Monitor",
        ...
        "log.stalled": "STOI: sesja {name} (PID {pid}) ma otwarta ture i nic nie zapisala od {d}. Najczesciej znaczy to, ze czeka pytanie o uprawnienia, ale jedno dlugie narzedzie albo czekanie na limit API wyglada stad tak samo. Nic sie nie wylaczy, dopoki tura jest otwarta.",
